...
class SheetTableVC: UITableViewController {
    ...
    let baseArr = [Model(title: "0、显示一个默认的 sheet 菜单", conten: ""),
                   Model(title: "1、显示一个带取消按钮的 sheet 菜单", conten: "可以添加多个输入框"),
                   Model(title: "2、显示一个不同控件顺序的 sheet 菜单", conten: "设置的顺序决定了控件显示的顺序"),
                   Model(title: "3、显示一个带自定义视图的 sheet 菜单", conten: "自定义视图的size发生改变时 会自动适应其改变."),
                   ...
                   Model(title: "11、显示多个加入队列和优先级的 sheet 弹框", conten: "当多个同时需要显示时, 队列和优先级决定了如何去显示"),
                   Model(title: "12、显示一个自定义动画配置的 sheet 弹框", conten: "可自定义打开与关闭的动画配置(UIView 动画)"),
                   Model(title: "13、显示一个自定义动画样式的 sheet 弹框", conten: "动画样式可设置动画方向, 淡入淡出, 缩放等")]
    
    let demoArr = [Model(title: "0、显示一个蓝色自定义风格的 sheet 弹框", conten: "弹框背景等颜色均可以自定义"),
                   Model(title: "1、显示一个分享登录的 sheet 弹框", conten: "类似某些复杂内容的弹框 可以通过封装成自定义视图来显示"),
                   Model(title: "2、显示一个提示打开推送的 sheet 弹框", conten: "类似某些复杂内容的弹框 可以通过封装成自定义视图来显示"),
                   Model(title: "3、显示一个提示签到成功的 sheet 弹框", conten: "类似某些复杂内容的弹框 可以通过封装成自定义视图来显示"),
                   Model(title: "4、显示一个单选选择列表的 sheet 弹框", conten: "类似某些复杂内容的弹框 可以通过封装成自定义视图来显示"),
                   Model(title: "5、显示一个省市区选择列表的 sheet 弹框", conten: "自定义的Action 通过设置其间距范围和边框等属性实现"),
                   Model(title: "6、显示一个评分的 sheet 弹框", conten: "自定义的Action 通过设置其间距范围和边框等属性实现"),
                   Model(title: "7、显示一个新手红包的 sheet 弹框", conten: "包含自定义视图 自定义title 自定义Action")]
